...
from .models import LedgerAccount, JournalEntry, WorkOrderInvoice, Payment
...
class FinancialReports:
    ...
    def get_balance_sheet(as_of_date=None):
        ...
        for entry in entries:
            ...
            if entry.debit_account.account_type in ['asset', 'expense']:
                account_balances[entry.debit_account] = account_balances.get(entry.debit_account, 0) + entry.amount
            else:
                account_balances[entry.debit_account] = account_balances.get(entry.debit_account, 0) - entry.amount

            if entry.credit_account.account_type in ['liability', 'equity', 'revenue']:
                account_balances[entry.credit_account] = account_balances.get(entry.credit_account, 0) + entry.amount
            else:
                account_balances[entry.credit_account] = account_balances.get(entry.credit_account, 0) - entry.amount

        # Organize by account type
        assets = {acc: bal for acc, bal in account_balances.items() if acc.account_type == 'asset'}
        liabilities = {acc: bal for acc, bal in account_balances.items() if acc.account_type == 'liability'}
        equity = {acc: bal for acc, bal in account_balances.items() if acc.account_type == 'equity'}
    ...
    def get_profit_loss(start_date=None, end_date=None):
        ...
        for entry in entries:
            if entry.debit_account.account_type == 'revenue':
                revenue_accounts[entry.debit_account] = revenue_accounts.get(entry.debit_account, 0) - entry.amount
            elif entry.credit_account.account_type == 'revenue':
                revenue_accounts[entry.credit_account] = revenue_accounts.get(entry.credit_account, 0) + entry.amount

            if entry.debit_account.account_type == 'expense':
                expense_accounts[entry.debit_account] = expense_accounts.get(entry.debit_account, 0) + entry.amount
            elif entry.credit_account.account_type == 'expense':
                expense_accounts[entry.credit_account] = expense_accounts.get(entry.credit_account, 0) - entry.amount
    ...
    def get_cash_flow(start_date=None, end_date=None):
        ...
        try:
            cash_account = LedgerAccount.objects.get(code='1000')  # Assuming cash account code
