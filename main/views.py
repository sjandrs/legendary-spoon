...
class RateLimitedPasswordResetView(auth_views.PasswordResetView):
	...
	def dispatch(self, request, *args, **kwargs):
		...
		if last_request and now - last_request < self.RATE_LIMIT_MINUTES * 60:
			return render(request, 'password_reset_form.html', {'form': self.get_form(), 'error': f'You can only request a password reset every {self.RATE_LIMIT_MINUTES} minutes.'})
...
def home(request):
    ...
    posts_by_author = Post.objects.values('author__username').annotate(count=Count('id')).order_by('-count')
