...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        from main.models import Contact, Deal
        contacts_with_deals = Contact.objects.filter(
            deals__isnull=False
        ).distinct()
        ...
        for contact in contacts_with_deals:
            try:
                ...
                clv_count += 1
                self.stdout.write(f'Calculated CLV for {contact.first_name} {contact.last_name}: ${clv.predicted_clv}')
            except Exception as e:
                self.stdout.write(self.style.WARNING(f'Failed to calculate CLV for {contact}: {e}'))
